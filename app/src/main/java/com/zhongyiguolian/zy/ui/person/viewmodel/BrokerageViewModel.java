 ...
 * @date 2020/12/10
 */
public class BrokerageViewModel extends CustomViewModel<MyRepository> {
     ...
     * @param model
     */
    public BrokerageViewModel(@NonNull Application application, MyRepository model) {
     ...
     */
    public ObservableList<BrokeragelItemViewModel> observableList = new ObservableArrayList<>();


    /**
     * 给RecyclerView添加ItemBinding
     */
    public ItemBinding<BrokeragelItemViewModel> itemBinding = ItemBinding.of(BR.viewModel, R.layout.item_brokerage);
        ...
        if(code == Constants.GETMYINCOME1){
            ...
            if(mList != null && mList.size() > 0){
                ...
                for(FilIncomeBean.MyIncomeDTO bean : mList){
                    ...
                    BrokeragelItemViewModel itemViewModel = new BrokeragelItemViewModel(this,bean);
                    observableList.add(itemViewModel);
                }

                //处理最后一个item底部阴影显示情况
                if(curPage != FIRST_PAGE){
                    for(BrokeragelItemViewModel itemViewModel : observableList){
